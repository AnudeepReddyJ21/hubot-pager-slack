  ...
  robot.respond /(pager|major)( me)? services$/i, (msg) ->
    ...
    pagerduty.get "/services", {}, (err, json) ->
      ...
      if services.length > 0
        ...
        msg.send "Here are the list of services \n" + buffer
      else
        msg.send 'No services found!'


  robot.respond /911 (.*)/i, (msg) ->
    poly = msg.match[1]
    if pagerduty.missingEnvironmentForApi(msg)
      return

    pagerduty.get "/escalation_policies/on_call", {}, (err, json) ->
      if err?
        robot.emit 'error', err, msg
        return

      escalation_policies = json.escalation_policies
      emails1 = ''
      emails2 = ''
      user1 = ''
      user2 = ''
      for escalation_policy in escalation_policies
        if escalation_policy.name is poly
          for call in escalation_policy.on_call
            if (call.level is 1) && (call.start is null)
              msg.reply "No one's on-call now :("
              break
            else if (call.level is 1) && (call.start != null)
              user1 += "#{call.user.email}"
              user1 = user1.replace('@duolingo.com','')
              emails1 += "you are the incident commander "
            else if (call.level is 2) && (call.start != null)
              user2 += "#{call.user.email}"
              user2 = user2.replace('@duolingo.com','')
              emails2 += "you are responsible for communications"
              robot.messageRoom "@#{user1}", emails1 + "for " + "#{poly}"
              robot.messageRoom "@#{user2}", emails2 + " #" + "#{poly}"
            else if (call.level is 2) && (call.start is null)  
              emails1 += "and also responsible for communications"
              robot.messageRoom "@#{user1}", emails1 + " #" + "#{poly}"
              break
